...
frappe.ui.form.on('User', 'change_owner', function(frm,cdt,cdn){
	var p = frm.doc;	
	var d = locals[cdt][cdn];
	if(p.new_owner){
		frappe.call({		
			method:"erpnext.selling.doctype.customer.customer.change_owner",
			args: {            
				name: p.customer,            
				value: p.new_owner
			},
			callback: function() {				
				...
				frm.set_value("current_owner", "");
				frm.set_value("new_owner", "");
			}
		})
	}else{
